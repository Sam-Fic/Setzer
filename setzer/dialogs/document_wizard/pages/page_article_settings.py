    ...
    def observe_view(self):
        def format_changed(combo, pspec):
            ...
            if selected != Gtk.INVALID_LIST_POSITION:
                self.current_values['article']['page_format'] = self.view.page_format_names[selected]

        def font_size_changed(row, pspec):
            self.current_values['article']['font_size'] = int(row.get_value())

        def option_toggled(row, pspec, option_name):
            self.current_values['article']['option_' + option_name] = row.get_active()

        def margin_changed(row, pspec, side):
            self.current_values['article']['margin_' + side] = row.get_value()

        def on_orientation_toggle(row, pspec):
            self.current_values['article']['is_landscape'] = row.get_active()
